...
const SubcategoryItem = ({ item }: SubcategoryItemProps) => {
	return (
		<Link
			href={item.full_path}
			className="subcategory-list-item flex cursor-pointer flex-col items-center justify-start rounded-md border shadow-md transition duration-300 ease-in-out hover:border-gray-300 hover:shadow-lg md:min-h-[320px]"
		>
			<div className="flex w-full flex-wrap justify-center p-2">
				<h2 className="mt-2 min-h-[55px] text-center text-sm font-bold md:text-xl">{item.name}</h2>
